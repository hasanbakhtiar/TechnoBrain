...
//   console.log(info.data[0].relationships.author.data.type);
...
console.log(phone[0].name);
console.log('name:');
for (let index = 0; index < phone.length; index++) {
    let li  = document.createElement('li');
    li.innerHTML = `${phone[index].name} : ${phone[index].model}(${phone[index].storage})`;
    const a = document.querySelector('ul');
    a.appendChild(li);
    console.log(`${phone[index].name} : ${phone[index].model}(${phone[index].storage}) `);
